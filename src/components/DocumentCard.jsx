...
const buildFields = (doc, fallbackName, fallbackCountry, fallbackAddress) => {
  const documentData = doc?.documentData || {};
  const nameOnDoc = documentData.Name || doc?.name || fallbackName || 'Not provided';
  const addressOnDoc = documentData.Address || doc?.address || fallbackAddress || 'Not provided';
  const number =
    documentData.PassportNumber ||
    documentData.DocumentNumber ||
    doc?.number ||
    'Not provided';
  const expiryDate = formatDate(documentData.ExpiryDate || doc?.expiryDate);
  // const issueDate = formatDate(documentData.IssueDate || doc?.issueDate);

  return [
    { label: 'Name on Document:', value: nameOnDoc },
    { label: 'Address on Document:', value: addressOnDoc },
    { label: 'Document Number:', value: number },
    { label: 'Expiry Date:', value: expiryDate },
    // { label: 'Issue Date:', value: issueDate },
    { label: 'Country:', value: documentData.Nationality || fallbackCountry || 'Not provided' },
  ];
};

const DocumentCard = ({ documents = [], fallbackName, countryName, address }) => {
  const safeDocuments = useMemo(() => (documents.length ? documents : []), [documents]);
  ...
  const goPrev = () => {
    ...
    setActiveIndex((prev) => (prev - 1 + safeDocuments.length) % safeDocuments.length);
  };

  const goNext = () => {
    if (!safeDocuments.length) return;
    setActiveIndex((prev) => (prev + 1) % safeDocuments.length);
  };
      ...
      <div className="relative overflow-hidden rounded-2xl sm:flex-row flex flex-col-reverse pb-[72px] border border-[#76757B] backdrop-blur-[30px]">
        <div className="absolute top-4 left-6 md:hidden flex items-center gap-3">
          <h2 className="text-white font-semibold text-lg">
            {safeDocuments.length
              ? safeDocuments[activeIndex]?.type ||
                safeDocuments[activeIndex]?.documentData?.DocumentType ||
                'Document'
              : 'No documents shared'}
          </h2>
        </div>
        ...
        >
          {safeDocuments.length ? safeDocuments.map((doc, index) => {
            const type =
              doc?.type || doc?.documentData?.DocumentType || 'Document';
            const image = doc?.filePath || doc?.image || 'public/images/passport.png';
            const fields = buildFields(doc, fallbackName, countryName, address);
            ...
            >
              ...
              <div className="flex flex-wrap flex-col pt-8 md:pt-0 items-center md:items-end gap-4">
                <div className="w-80 md:h-28 lg:h-36 h-36  rounded-lg overflow-hidden border border-[#76757B]">
                  <img
                    src={image}
                    alt={doc?.imageAlt || type}
                    className="h-full w-full object-cover"
                  />
                </div>
                <div className="text-xs text-white">
                  {index + 1} / {safeDocuments.length}
                </div>
              </div>
            </div>
          );}) : (
            <div className="min-w-full p-6 text-white text-center">
              No identity documents shared.
            </div>
          )}
        </div>
